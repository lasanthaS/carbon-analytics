    ...
    public void testFilterQuery() {
        ...
        try {
            ...
            InputHandler tempStreamHandler =
                    siddhiAppRuntimeMap.get("TestPlan2-001").get(0)
...
//    @Test(dependsOnMethods = "testPartitionWithWindow")
     ...
     * {@link TransportStrategy#FIELD_GROUPING} The unpartitioned streams in the join will
    ...
    public void testPartitionStrategy() {
        ...
        try {
            Map<String, List<SiddhiAppRuntime>> siddhiAppRuntimeMap =
                    createSiddhiAppRuntimes(siddhiManager, queryGroupList);
            InputHandler tempStreamHandler =
                    siddhiAppRuntimeMap.get("TestPlan7-group1").get(0)
                            .getInputHandler("TempStream");
     ...
     * same Partition key gets assigned with the respective parallelism as as distinct
    ...
    public void testPartitionMultiSubscription() {
        ...
        try {
            Map<String, List<SiddhiAppRuntime>> siddhiAppRuntimeMap =
                    createSiddhiAppRuntimes(siddhiManager, queryGroupList);
            InputHandler stockStreamHandler = siddhiAppRuntimeMap.get("TestPlan8-000").get(0)
                    .getInputHandler("stockStream");
            InputHandler triggerStreamHandler = siddhiAppRuntimeMap.get("TestPlan8-000").get(0)
            ...
            SiddhiTestHelper.waitForEvents(1000, 2, dumbStreamCount,
                    2000);
    ...
    public void testPartitionWithMultiKey() {
        ...
        try {
            ...
            InputHandler stockStreamHandler = siddhiAppRuntimeMap.get("TestPlan9-001").get(0)
                    .getInputHandler("stockStream");
            InputHandler triggerStreamHandler = siddhiAppRuntimeMap.get("TestPlan9-001").get(0)
                    .getInputHandler("companyTriggerStream");
    ...
    public void testSinkStreamForSource() {
        ...
        try {
            Map<String, List<SiddhiAppRuntime>> siddhiAppRuntimeMap =
                    createSiddhiAppRuntimes(siddhiManager, queryGroupList);
            InputHandler stockStreamHandler = siddhiAppRuntimeMap.get("TestPlan11-001").get(0)
                    .getInputHandler("stockStream");
    ...
    public void testUsergivenParallelSources() {
        String siddhiApp = "@App:name('TestPlan12') \n"
                ...
                + "@source(type='inMemory', topic='stock123', @map(type='json'),"
                + " @dist(parallel='3'))  "
                + "Define stream stockStream(symbol string, price float, quantity int,"
                + " tier string);\n"
                + "@source(type='inMemory', topic='companyTrigger', @map(type='json'))"
        ...
        Assert.assertTrue(queryGroupList.get(1).isReceiverQueryGroup(),
                "Receiver type should be set");
        Assert.assertTrue(queryGroupList.get(2).getGroupName()
                        .contains(SiddhiTopologyCreatorConstants.PASSTHROUGH),
                "Two passthrough queries should be present in separate groups");
        Assert.assertTrue(queryGroupList.get(2).isReceiverQueryGroup(),
                "Receiver type should be set");
    }
    ...
    public void testPassthoughWithMultipleSubscription() {
        ...
        Assert.assertTrue(queryGroupList.get(0).getSiddhiQueries().get(0).getApp()
                        .contains("@source(type = 'http', receiver.url='http://localhost:8080"
                                + "/SweetProductionEP',"
                                + " @map(type = 'json'))\n"
    ...
    public void testUsergivenSourceSingleGroup() {
        ...
        Assert.assertTrue(queryGroupList.size() == 2,
                "Two query groups should be created");
        Assert.assertTrue(queryGroupList.get(0).getGroupName()
                        .contains(SiddhiTopologyCreatorConstants.PASSTHROUGH),
                "passthrough query should be present in a separate group");
        Assert.assertTrue(queryGroupList.get(0).isReceiverQueryGroup(),
    ...
    public void testMultipleRRstrategy() {
        String siddhiApp = "@App:name('TestPlan25')\n"
                ...
                + "select name,amount\n"
                + "insert into Test3Stream;\n"
                + "@info(name = 'query3')@dist(parallel='3',execGroup='003')\n"
                + "from Test2Stream\n"
                + "select name,amount\n"
        ...
        Assert.assertTrue(queryGroupList.get(0).getSiddhiQueries().get(0).getApp()
                        .contains("@App:name('TestPlan25-001-1') \n"
